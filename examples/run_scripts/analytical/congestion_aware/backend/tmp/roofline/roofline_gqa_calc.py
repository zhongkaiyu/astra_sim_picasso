...
def roofline_mla_qkv(peak_perf, bw, cfg, bs):
    ...
    heads_per_npu = Hq // n_npus          # 128/16 = 8
...
def roofline_mla_output(peak_perf, bw, cfg, bs):
    ...
    # Per-NPU weight: Hq × d_model × d_value_shard (W_O_abs 的 1/N 切片)
    w_elems = Hq * d * d_value_shard
    mac = bs * Hq * d * d_value_shard
    act_elems = bs * (Hq * d_value_shard + d)
    total_mem = w_elems + act_elems
...
def calc_mla_strategy(hw_cfg, bs, seq_list, link_bw=1.5,
    ...
    # 每 NPU 负责计算的 heads 数 (权重切分)
    heads_per_npu_weight = Hq // n_npus   # 128/16 = 8
